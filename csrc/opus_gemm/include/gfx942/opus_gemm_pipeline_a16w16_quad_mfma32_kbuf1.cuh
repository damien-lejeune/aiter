...
// gfx942 dedicated 256x256x{32,64} / 4-wave / 32x32x8-MFMA pipeline.
...
__global__ __launch_bounds__(Traits::BLOCK_SIZE, 1) void gemm_a16w16_quad_mfma32_kbuf1_kernel(Kargs kargs) {
    ...
    static_assert(T::B_M == 256 && T::B_N == 256 && (T::B_K == 32 || T::B_K == 64));
    static_assert(T::T_M == 2 && T::T_N == 2 && T::T_K == 1);
    static_assert(T::W_M == 32 && T::W_N == 32 && T::W_K == 8);
    static_assert(T::E_M == 2 && T::E_N == 2 && (T::E_K == 4 || T::E_K == 8));
    ...
    static_assert((N_RA == 4 || N_RA == 8) && N_RA == N_RB);
    ...
    auto run_steady_tile_ek4 = [&](auto& cur_a, auto& cur_b, auto& next_a, auto& next_b, int tile,
    ...
    auto run_steady_tile_ek8 = [&](auto& cur_a, auto& cur_b, auto& next_a, auto& next_b, int tile,
                                   auto& s_a_next0, auto& s_a_next1,
                                   auto& s_b_next0, auto& s_b_next1,
                                   const unsigned* next_lds_a0, const unsigned* next_lds_a1,
                                   const unsigned* next_lds_b0, const unsigned* next_lds_b1) {
        ...
        s_waitcnt_lgkmcnt(0_I);
        phase_compute_2x2<T>(cur_a[0], cur_b[0], acc_00);
        phase_compute_2x2<T>(cur_a[0], cur_b[1], acc_01);
        phase_compute_2x2<T>(cur_a[1], cur_b[0], acc_10);

        s_waitcnt_vmcnt(0_I);
        #pragma unroll
        for (int i = 0; i < N_RA; i++) {
            s_a_next0.template store_part<T::VEC_A>(pf_a0, u_sa, i);
        }
        #pragma unroll
        for (int i = 0; i < N_RB; i++) {
            s_b_next0.template store_part<T::VEC_B>(pf_b0, u_sb, i);
        }
        #pragma unroll
        for (int i = 0; i < N_RA; i++) {
            s_a_next1.template store_part<T::VEC_A>(pf_a1, u_sa, i);
        }
        #pragma unroll
        for (int i = 0; i < N_RB; i++) {
            s_b_next1.template store_part<T::VEC_B>(pf_b1, u_sb, i);
        }
    ...
    auto run_steady_tile = [&](auto& cur_a, auto& cur_b, auto& next_a, auto& next_b, int tile,
                               ...
                               const unsigned* next_lds_a0, const unsigned* next_lds_a1,
                               const unsigned* next_lds_b0, const unsigned* next_lds_b1) {
        if constexpr (T::E_K == 4) {
            run_steady_tile_ek4(
                cur_a, cur_b, next_a, next_b, tile,
                s_a_next0, s_a_next1, s_b_next0, s_b_next1,
                next_lds_a0, next_lds_a1, next_lds_b0, next_lds_b1);
        } else {
            run_steady_tile_ek8(
                cur_a, cur_b, next_a, next_b, tile,
    ...
    auto run_tail_tile_ek4 = [&](auto& cur_a, auto& cur_b, auto& next_a, auto& next_b,
                                 auto& s_a_next0, auto& s_a_next1,
                                 auto& s_b_next0, auto& s_b_next1,
                                 const unsigned* next_lds_a0, const unsigned* next_lds_a1,
                                 const unsigned* next_lds_b0, const unsigned* next_lds_b1) {
    ...
    auto run_tail_tile_ek8 = [&](auto& cur_a, auto& cur_b, auto& next_a, auto& next_b,
                                 auto& s_a_next0, auto& s_a_next1,
                                 auto& s_b_next0, auto& s_b_next1,
                                 const unsigned* next_lds_a0, const unsigned* next_lds_a1,
                                 const unsigned* next_lds_b0, const unsigned* next_lds_b1) {
